...
Future<List<Map<String, dynamic>>> getMovies() async {
  ...
  var results = await conn.query('SELECT id, title, genre, rating, cover_image_url FROM movies');
  ...
  for (var row in results) {
    movies.add({
      ...
      'genre': row[2],
      'rating': row[3],
      'cover_image_url': row[4], // افزودن فیلد کاور عکس
    });
  }
  await conn.close();
  return movies;
...
router.post('/movies', (Request request) async {
  try {
    ...
    var coverImageUrl = data['cover_image_url']; // دریافت لینک کاور

    var conn = await connectToDatabase();
    await conn.query(
      'INSERT INTO movies (title, genre, rating, cover_image_url) VALUES (?, ?, ?, ?)',
      [title, genre, rating, coverImageUrl], // ذخیره لینک کاور در دیتابیس
    );
    await conn.close();
